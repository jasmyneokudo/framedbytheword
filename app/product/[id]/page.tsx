...
function loadBatch(): BatchState {
  if (typeof window === "undefined") return { endsAt: Date.now() + BATCH_DURATION_MS, claimed: {} };
  try {
  ...
  const fresh: BatchState = { endsAt: Date.now() + BATCH_DURATION_MS, claimed: {} };
  try { localStorage.setItem(BATCH_KEY, JSON.stringify(fresh)); } catch {}
  return fresh;
}

function saveBatch(b: BatchState) {
  try { localStorage.setItem(BATCH_KEY, JSON.stringify(b)); } catch {}
}
...
function tiersFor(productClaimed: number): Tier[] {
  ...
  return [
    ...
    { label: "Next 10 Orders", discount: 0.10, capacity, claimed: t2c },
    { label: "Next 10 Orders", discount: 0.05, capacity, claimed: t3c },
    { label: "Full Batch Pricing", discount: 0, capacity: Infinity, claimed: 0 },
  ];
}
...
//   const navigate = useNavigate();
  ...
  useEffect(() => {
    const cities = ["Abuja", "Lagos", "Port Harcourt", "Ibadan", "Kano", "Enugu"];
    const messages = [
      (c: string) => `Someone in ${c} just secured the 15% early-buyer discount.`,
  ...
  const claimedThisMonth = useMemo(
    () => Object.values(batch.claimed).reduce((a, b) => a + b, productClaimed > 6 ? 0 : 6) + 42,
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-2">
          ...
          <div>
            ...
            <div className="mt-6 rounded-lg border border-gold/40 bg-gold/5 p-5">
              <p className="font-serif text-lg text-foreground">You&apos;re Early.</p>
              <p className="mt-1 font-sans text-sm text-muted-foreground">
                Congratulations — you&apos;ve arrived before this month&apos;s production batch begins.
                Secure your production slot today and save up to <span className="font-medium text-foreground">15%</span> before production starts.
              </p>
            </div>
            ...
            <div className="mt-6 rounded-lg border border-border bg-cream/40 p-6">
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <p className="font-sans text-[10px] uppercase tracking-widest text-muted-foreground">Regular</p>
                  <p className="mt-1 font-serif text-lg text-muted-foreground line-through">{formatNaira(regularPrice)}</p>
                </div>
                <div>
                  <p className="font-sans text-[10px] uppercase tracking-widest text-gold">Your price</p>
                  <p className="mt-1 font-serif text-2xl font-medium text-foreground">{formatNaira(yourPrice)}</p>
                </div>
                <div>
                  <p className="font-sans text-[10px] uppercase tracking-widest text-muted-foreground">You save</p>
                  <p className="mt-1 font-serif text-2xl font-medium text-gold">{formatNaira(saving)}</p>
                </div>
              </div>
              {active.discount > 0 && (
                <p className="mt-4 font-sans text-xs text-muted-foreground">
                  <TrendingDown className="mr-1 inline h-3.5 w-3.5 text-gold" />
                  Only {active.capacity - active.claimed} slots left at {(active.discount * 100).toFixed(0)}% off. Discount decreases when this tier fills.
                </p>
              )}
            </div>
      ...
      <section className="border-y border-border bg-primary text-primary-foreground py-14">
        <div className="mx-auto max-w-4xl px-6 text-center">
          <p className="font-sans text-[11px] uppercase tracking-[0.35em] text-gold">Current Production Batch</p>
          <h2 className="mt-3 font-serif text-2xl font-medium md:text-3xl">Production starts in</h2>
          <div className="mt-8 grid grid-cols-4 gap-3 md:gap-6">
            {[
              ...
              <div key={t.l} className="rounded-lg border border-primary-foreground/15 bg-primary-foreground/5 py-6">
                <p className="font-serif text-3xl font-medium md:text-5xl">{String(t.v).padStart(2, "0")}</p>
                <p className="mt-2 font-sans text-[10px] uppercase tracking-widest text-primary-foreground/70">{t.l}</p>
              </div>
            ))}
          </div>
          <p className="mt-8 font-sans text-sm text-primary-foreground/80">
            Orders placed before production begins qualify for exclusive early-buyer pricing.
          </p>
        </div>
      </section>
      ...
      <section className="py-20">
        <div className="mx-auto max-w-6xl px-6">
          <div className="text-center">
            <p className="font-sans text-[11px] uppercase tracking-[0.3em] text-gold">Early-Buyer Pricing</p>
            <h2 className="mt-3 font-serif text-3xl font-medium text-foreground">Discount decreases as slots fill</h2>
          </div>
          <div className="mt-12 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
            {tiers.map((t, i) => {
              const isActive = t === active;
              const locked = i > tiers.indexOf(active);
              const filled = t.capacity === Infinity ? 0 : (t.claimed / t.capacity) * 100;
              ...
              return (
                ...
                >
                  ...
                  <p className="font-sans text-[11px] uppercase tracking-widest text-muted-foreground">{t.label}</p>
                  <p className="mt-2 font-serif text-3xl font-medium text-foreground">
                    {t.discount > 0 ? `${(t.discount * 100).toFixed(0)}% OFF` : "Full Price"}
                  </p>

                  <div className="mt-4 space-y-1 font-sans text-sm">
                    <div className="flex justify-between text-muted-foreground">
                      <span>Regular</span>
                      <span className="line-through">{formatNaira(regularPrice)}</span>
                    </div>
                    <div className="flex justify-between text-foreground">
                      <span>Your price</span>
                      <span className="font-medium">{formatNaira(tierPrice)}</span>
                    </div>
                    <div className="flex justify-between text-gold">
                      <span>You save</span>
                      <span className="font-medium">{formatNaira(regularPrice - tierPrice)}</span>
                    </div>
                  </div>
      ...
      <section className="border-y border-border bg-cream/30 py-20">
        <div className="mx-auto max-w-5xl px-6">
          <div className="text-center">
            <p className="font-sans text-[11px] uppercase tracking-[0.3em] text-gold">The Math</p>
            <h2 className="mt-3 font-serif text-3xl font-medium text-foreground">Buy today vs. wait</h2>
          </div>
          <div className="mt-12 grid grid-cols-1 gap-6 md:grid-cols-2">
            <div className="rounded-lg border-2 border-gold bg-background p-8">
              <p className="font-sans text-[11px] uppercase tracking-widest text-gold">Buy Today</p>
              <p className="mt-3 font-serif text-4xl font-medium text-foreground">Save {formatNaira(saving)}</p>
            ...
            <div className="rounded-lg border border-border bg-muted/40 p-8">
              <p className="font-sans text-[11px] uppercase tracking-widest text-muted-foreground">Wait Until Later</p>
              <p className="mt-3 font-serif text-4xl font-medium text-muted-foreground">Pay {formatNaira(saving)} more</p>
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
            {[
              ...
              <div key={s.l} className="rounded-lg border border-border bg-card p-8 text-center">
                ...
                <p className="mt-4 font-serif text-3xl font-medium text-foreground">{s.v}</p>
                <p className="mt-1 font-sans text-sm text-muted-foreground">{s.l}</p>
              </div>
            ))}
          </div>
          <p className="mt-12 text-center font-serif text-xl italic text-muted-foreground">
            &quot;Join hundreds of families filling their homes with God&apos;s Word.&quot;
          </p>
        </div>
      </section>
      ...
      <section className="border-t border-border bg-primary/5 py-20">
        <div className="mx-auto max-w-3xl px-6 text-center">
          <p className="font-sans text-[11px] uppercase tracking-[0.3em] text-gold">Why We Do This</p>
          <h2 className="mt-3 font-serif text-3xl font-medium text-foreground">Production batches, not mass production</h2>
          <p className="mt-6 font-sans text-base font-light leading-relaxed text-muted-foreground">
            Every FramedWithTheWord piece is produced in carefully planned production batches to ensure
            exceptional quality, attention to detail, and timely delivery. Early-buyer pricing rewards customers
            who reserve their production slots before production begins. The earlier you order, the more you save.
          </p>
        </div>
      </section>
            ...
            {PRODUCTS.filter((p) => p.id !== product?.id).slice(0, 4).map((p) => (
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <p className="mt-0.5 font-sans text-[10px] uppercase tracking-widest text-gold">{p.reference}</p>
                </div>
              </Link>
            ))}
          </div>
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 border-t border-border bg-background/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl flex-col items-stretch gap-3 px-4 py-3 md:flex-row md:items-center md:justify-between md:px-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="font-sans text-[10px] uppercase tracking-widest text-gold">
                Save {formatNaira(saving)} · {active.capacity === Infinity ? "Full price" : `${active.capacity - active.claimed} slots left`}
              </p>
              <p className="font-serif text-sm text-foreground">
                <span className="text-muted-foreground line-through mr-2">{formatNaira(regularPrice)}</span>
          ...
          <div className="flex items-center gap-3">
            ...
            <Button onClick={() => setShowReserve(true)} className="bg-primary text-primary-foreground hover:bg-primary/90">
              <Lock className="mr-2 h-4 w-4" />
              Lock My Slot
            </Button>
          </div>
      ...
      {toast && (
        <div className="fixed bottom-24 left-4 z-40 max-w-xs animate-fade-in rounded-lg border border-border bg-background/95 p-4 shadow-lg backdrop-blur">
          <p className="font-sans text-[10px] uppercase tracking-widest text-gold">Live activity</p>
      ...
      {/* Reservation modal */}
      {showReserve && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => setShowReserve(false)}>
          <div className="w-full max-w-md rounded-lg bg-background p-8 shadow-2xl animate-scale-in" onClick={(e) => e.stopPropagation()}>
            <p className="font-sans text-[11px] uppercase tracking-[0.3em] text-gold">Reserve Your Discount</p>
            <h3 className="mt-2 font-serif text-2xl font-medium text-foreground">Lock your production slot</h3>
            <p className="mt-2 font-sans text-sm text-muted-foreground">
              We'll hold your {(active.discount * 100).toFixed(0)}% discount and {selectedSize.label} slot for 15 minutes while you complete checkout.
            </p>
            <form onSubmit={submitReserve} className="mt-6 space-y-4">
              {[
                ...
                { k: "whatsapp", label: "WhatsApp number", type: "tel", placeholder: "+234 ..." },
              ].map((f) => (
                <div key={f.k}>
                  <label className="block font-sans text-[10px] uppercase tracking-widest text-muted-foreground mb-1">{f.label}</label>
                  <input
                    required
                    type={f.type}
              ...
              <Button type="submit" size="lg" className="w-full bg-primary text-primary-foreground hover:bg-primary/90">
                Reserve My Early-Buyer Price
              </Button>
              <p className="text-center font-sans text-[11px] text-muted-foreground">
                Abuja delivery is payable on delivery. Outside Abuja shipping is added at checkout.
              </p>
            </form>
          </div>
        </div>
      ...
      {reserved && !reservation.done && items.length !== 0 && (
        <div className="fixed top-20 right-4 z-40 rounded-lg border border-gold bg-background/95 p-4 shadow-lg backdrop-blur animate-fade-in">
          ...
          <p className="mt-1 font-mono text-lg text-foreground">
            {String(reservation.m).padStart(2, "0")}:{String(reservation.s).padStart(2, "0")}
          </p>
